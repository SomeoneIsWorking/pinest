...
def _recv_json(sock: socket.socket, deadline: float) -> dict[str, Any] | None:
    while time.time() < deadline:
        ...
        if not chunk:
            return None
        try:
            return json.loads(chunk.decode(errors="replace"))
    ...
    for port in ports:
        try:
            with socket.create_connection(("127.0.0.1", port), timeout=5) as sock:
                ...
                if not reply:
                    failures.append(f"{port}: no authentication reply")
                    continue
                if reply.get("type") == "error" or reply.get("authed") is False:
                    failures.append(f"{port}: {reply.get('message', 'authentication rejected')}")
                    continue
