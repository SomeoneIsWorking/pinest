...
Fixed at the transport, not in the renderer. The goal directive travels as pi's custom message pinest-goal (goalAppMessage) and a peer-session message as pinest-message (peerMessage, with details.from/fromId so the card names the sender); Supervisor.deliverInjectedMessage is the single spawned-session transport and the host uses pi.sendMessage, both fire-and-forget with a reported failure. History carries details through custom_message entries. The app renders InjectedMessageCard — left-aligned, accent bar, 'PiNest · goal' / 'PiNest · message from <session>' — never the user's bubble styling. Evidence: session-goal-routing.test.ts (a goal is never delivered as a user message; it reaches the agent as a custom message), session-messaging.test.ts (peer messages carry the custom type and sender), and injected_message_test.dart's discriminator: the identical directive text as role 'user' renders a MessageBubble and no card, so the distinction is by type, never by wording. Text sniffing was rejected — attribution by id/type is already the project rule.
