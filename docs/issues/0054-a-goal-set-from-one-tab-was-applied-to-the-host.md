...
Fixed end to end. The goal is per-session state: it lives on SessionRow.goal + SessionSnapshot.goal (the top-level state.goal and config.goal are gone; a legacy host-wide value is retired on load because it names no session). setSessionGoal/clearSessionGoal are the only writers, through one GoalSink that persists and publishes the same value; normalizeGoal is the only reader. goal_set/goal_clear are session commands that REQUIRE sessionId, so an untargeted goal is refused by validation instead of landing on the host, and the app sends the tab's session id. Evidence: session-goal-routing.test.ts drives both real handlers (a spawned session's goal is stored, published, and handed to that session's agent as a custom message; the host handler sets the host's own), command-validation.test.ts (an untargeted goal touches no session; a targeted one reaches exactly that one), session-goal.test.ts (setting one session leaves others alone), session_store_test.dart + widget_test.dart (the banner appears only on the tab that owns the goal).
